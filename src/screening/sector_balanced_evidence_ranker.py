...
def _prepare_raw_ranking(raw_ranking_df: pd.DataFrame, policy: dict[str, Any]) -> pd.DataFrame:
    ...
    score_source = raw["evidence_priority_score"] if "evidence_priority_score" in raw.columns else raw.get("raw_evidence_priority_score", pd.Series(0, index=raw.index))
    raw["raw_evidence_priority_score"] = pd.to_numeric(score_source, errors="coerce").fillna(0)
