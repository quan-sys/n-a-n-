"""Defensive vietfin market adapter for 04C provider probes."""
...
def fetch_vietfin_market(
    *,
    ...
    fetcher = _detect_fetcher(module)
    if fetcher is None:
        return [
            _attempt_row(ticker, source_family, "PROVIDER_API_UNSUPPORTED", False, True, "NoSupportedFetcher", "No explicit vietfin OHLCV/history fetch interface was detected.")
...
def _detect_fetcher(module: Any):
...
def _last_market_point(frame: Any) -> dict[str, Any]:
    ...
    adjusted_col = _first_column(data, ["adjusted_close", "adj_close"])
